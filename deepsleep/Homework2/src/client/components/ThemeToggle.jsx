...
export default function ThemeToggle() {
    ...
    return (
        <button
            onClick={toggleTheme}
            className="p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors border border-white/20 backdrop-blur-sm"
            title={theme === 'dark' ? "עבור למצב אור" : "עבור למצב חושך"}
        >
            {theme === 'dark' ? (
                // Sun Icon
