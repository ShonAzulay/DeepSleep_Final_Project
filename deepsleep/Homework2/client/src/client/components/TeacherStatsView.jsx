...
export default function TeacherStatsView({ experimentId, classId, onBack }) {
    ...
    useEffect(() => {
        async function load() {
            try {
                // Fetch only for this specific class
                const entries = await fetchClassSleepEntries(experimentId, classId);
        ...
        return (
            <SpaceLayout>
                <div className="min-h-screen flex items-center justify-center">
                    <p className="text-emerald-400 text-xl font-bold animate-pulse">טוען נתוני כיתה...</p>
                </div>
            </SpaceLayout>
        );
    }
        ...
        <SpaceLayout>
            <div className="max-w-7xl mx-auto p-4 pb-20">
                <div className="flex justify-between items-center mb-8">
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]">סטטיסטיקה כיתתית</h1>
                    <button
                        onClick={onBack}
                        className="rounded-xl border border-emerald-500/50 px-6 py-2 text-emerald-300 font-semibold hover:text-white hover:bg-white/5 transition-colors"
                    >
                        חזרה לדשבורד
                    </button>
                </div>
                ...
                <div className="flex justify-center mb-8">
                    <GlassCard className="text-center py-6 px-12 transform hover:scale-105 transition-transform duration-300" glowColor="emerald">
                        <div className="text-5xl font-black text-white drop-shadow-[0_0_10px_rgba(16,185,129,0.5)]">{stats?.totalEntries}</div>
                        <div className="text-emerald-300 text-sm mt-2 uppercase tracking-wider font-bold">סה"כ דיווחים בכיתה</div>
                    </GlassCard>
                </div>
